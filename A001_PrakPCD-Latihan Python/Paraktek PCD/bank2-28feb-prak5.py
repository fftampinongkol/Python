...
def histogray (image):
    ...
    buckets = [0] * 300
    arraynorm = [0] * 300
    scale = 1 #belum di gunakan
    #dimensi kanvas histogram
    histocol= 255
    historow= 150
    border = 30 #border atas

    kanvashisto = np.zeros(((historow+border),histocol,1), np.uint8) # inisialisasi kanvas
    row, col , raw = image.shape
    for i in range(0,row):
        for j in range(0,col):
            buckets[int(image[i,j])]+=1 #mengumpulkan frekuensi intensitas
    maks = max(buckets)#mencari max dan min untuk menormalisasi frekuensi
    mins = min(buckets)
    for intent in range(0,255):
        jumlahperbar = buckets[intent] #membaca frekuensi tiap intensitas
        normal = int(float(jumlahperbar)/float(maks)*float(historow)) #menormalisasi nilai intensitas agar muat pada kanvas histogram
        arraynorm[intent] = normal # ga penting, hanya untuk debuging..
        #for x in range(0,scale):
        for y in range(int(historow-normal+border), historow+border):
            kanvashisto.itemset((y,intent,0),255) #draw di kanvas histogram
